	...
	for example_index, example in enumerate(examples):
		...
		for ending_index, ending in enumerate(example.endings):
			...
			if pad_on_left:
				...
				print("input_ids is {}\n".format(input_ids))#input_ids = tokenizer.convert_tokens_to_ids(input_ids)
				exit()#token_type_ids = (len(context_tokens_choice) + len(question_tokens) + 1) * [0] + (len(ending_tokens) + 2) * [1] + cls_segment_id
			else:
