...
class ARCProcessor(DataProcessor):
	...
	def get_train_examples(self, data_dir,part_train,index_file):
		logger.info("LOOKING AT {} train".format(data_dir))
		return self._create_train_examples(self._read_csv(os.path.join(data_dir,"Train_Final_complete.csv")),part_train,index_file, type="train")
	...
	def _create_train_examples(self, lines: List[List[str]],p, index_file, type:str):
		# Create examples for training and dev set
		...
		for ind in range(idx.shape[0]):
			index.append(idx[ind][0])
		#idx = np.random.choice(n,p, replace=False)
		print("Input idx {} \n".format(idx))
		xy = np.random.randint(low=1, high=100, size=1)
		repo_path = os.path.dirname(os.path.abspath(__file__))
		out_sw_2_fp = repo_path + "/index_list"+str(z)+"_"+str(xy)+".txt"
