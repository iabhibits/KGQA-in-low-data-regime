...
class InputFeatures(object):
	def __init__(self, example_id, choices_features, label):
		...
		self.choices_features = [
			...
			for input_ids, attention_mask, token_type_ids,concept, adj_matrix, in choices_features
		]
		self.label = label
...
class ARCProcessor(DataProcessor):
	...
	def get_train_examples(self, data_dir,part_train,file_index):
		logger.info("LOOKING AT {} train".format(data_dir))
		return self._create_train_examples(self._read_csv(os.path.join(data_dir,"Train_Final_complete.csv")),part_train,file_index, type="train")
	...
	def _create_examples(self, lines: List[List[str]], type:str):
		...
		return examples
	def _create_train_examples(self, lines: List[List[str]],p, file_index, type:str):
		# Create examples for training and dev set
		...
		n = len(lines[1:])
		#print("##############################################################################")
		print("total length of the examples: {} \n".format(n))
		z = p
		p = int(p * n)
		...
		#print('\n')
		#print("Input idx {} \n".format(idx))
		xy = np.random.randint(low=1, high=1000, size=1)
		#repo_path = os.path.dirname(os.path.abspath(__file__))
		print("Random No is {}\n".format(xy))
		#out_sw_2_fp = "/index_list"+str(z)+"_"+str(file_index)+".txt"
		#np.savetxt(out_sw_2_fp, idx, fmt="%s")
		print("###########################################################################")
		# for i in idx:
		# 	line = lines[i+1]
		# 	line = lines[i+1]
...
def read_graph_example(args,typ):
	...
	if typ == 'train':
		graph_ngx_file = args.path_to_pathgraph + "final_sim_pre_hyp_graph.pnxg"
	elif typ == 'val':
		graph_ngx_file = args.path_to_pathgraph + "final_sim_easy_graph.pnxg"
	elif typ == 'test':
		graph_ngx_file = args.path_to_pathgraph + "final_sim_test_graph.pnxg"
	...
	if typ == 'train':
		save_file = args.path_to_graph+"graph_train.pnxg.dgl.pk"
	elif typ == 'val':
		save_file = args.path_to_graph +"graph_easy.pnxg.dgl.pk"
	elif typ == 'test':
		save_file = args.path_to_graph +"graph_test.pnxg.dgl.pk"
	reload=True
...
def convert_examples_to_features(args,
	...
	if typ == 'train':
		...
		mat_file = args.path_to_graph+"easy_matrix.pt"
		con_file = args.path_to_graph+"easy_concept.pt"
	elif typ == 'test':
	...
	for example_index, example in enumerate(examples):
		...
		for ending_index, ending in enumerate(example.endings):
			...
			choices_features.append((input_ids, attention_mask, token_type_ids, concept, adj_matrix))
