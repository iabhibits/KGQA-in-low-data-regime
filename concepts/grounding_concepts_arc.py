...
def proces_arc(args,data):
    ...
    output_path = args.output_dir+"arc_concepts_"+str(args.typ)+str(args.start)+"_"+str(args.end)+".mcp"
    concepts = []
    examples = data[args.start:args.end]
    for i,example in tqdm(enumerate(examples),total=len(examples)):
        x = []
        for j in tqdm(range(4)):
            res = match_mentioned_concepts_context(nlp,context_sents=example.context_sentences[j],
...
def main():
    ...
    parser.add_argument("--typ",type=str,required=True, help=" Type of dataset i.e., [ train, easy-test or challenge_test]",)
